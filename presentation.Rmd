...
- This is not well readable, because one extreme outlier dictate the scale
- A 15kg penguin is a scary thought and we would probably remove this outlier, but let's assume that's a valid observation we want to include in the plot
...
peng %>%
  ...
  dplyr::summarise(
    min_age = min(body_mass_g, na.rm = T),       # a new column: min weight for each group
    median_age = median(body_mass_g, na.rm = T), # a new column: median weight for each group
    max_age = max(body_mass_g, na.rm = T)        # a new column: max weight for each group
  )
```
...
peng %>% 
  ...
  dplyr::mutate(                               # mutate does not collapse, unlike summarise
...
- `dplyr::case_when` is a more readable solution for this application
...
- Contains additional variables for a subset of penguins - even some not in the main dataset
...
left <- dplyr::left_join(
  x = peng,    # 1060 observations
  y = bills,  # 1657 observations
  by = c("id") # the key columns by which to join
)
```
